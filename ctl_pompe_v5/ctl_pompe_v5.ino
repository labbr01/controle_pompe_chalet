...
/*
 * ============================================================================
 * VERSION 5: LCD I2C 20x4 (Freenove)
 * Remplace OLED défectueux par LCD I2C robuste
 * ============================================================================
 * 
 * Branchement LCD I2C:
 * - SDA → A4
...
LiquidCrystal_I2C lcd(0x27, 20, 4); // Changez à 0x3F si l'écran ne s'allume pas
...
// ============================================================================
// CONSTANTES DE CONFIGURATION - BUFFERS
// ============================================================================
...
void gestionAutomatique() {
  ...
  if (indexPile < TAILLE_PILE || ampIndex < N_LISSAGE) {
    ...
    if (!traceInit) {
      Serial.println("[INIT] Remplissage des buffers...");
      traceInit = true;
    }
    return;
  }
  ...
  if (purgeActive) {
    ...
    if (debit > 0 || pression > SEUIL_PRESSION_PURGE) {
      purgeActive = false;
      digitalWrite(RELAIS_PURGE, LOW);
      Serial.println("[FIN PURGE] Debit/pression OK");
